...
async function deployAutomatedApiConsumer(chainId = network.config.chainId) {
  ...
  const { success, resultLog } = await simulateRequest(
  ...
  if (chainId == 31337) {
    ...
    const mockOracleFactoryFactory = await ethers.getContractFactory(
      "FunctionsOracleFactory"
    );
    mockOracleFactory = await mockOracleFactoryFactory
      .connect(deployer)
      .deploy();
    ...
    const FunctionsOracleAddress =
      OracleDeploymentReceipt.events[0].args.oracle;
    mockOracle = await ethers.getContractAt(
      "FunctionsOracle",
  ...
  console.log(
    `AutomatedAPIConsumer deployed to ${apiConsumer.address} on ${network.name}`
  );
