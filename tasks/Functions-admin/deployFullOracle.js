...
task(
  ...
  console.log(
    `FunctionsBillingRegistry deployed to ${registry.address} on ${network.name}`
  );
  ...
  console.log(
    `Setting DON public key to ${
      networkConfig[network.name]["functionsPublicKey"]
    }`
  );
  overrides
    ? await oracle.setDONPublicKey(
        "0x" + networkConfig[network.name]["functionsPublicKey"],
        overrides
      )
    : await oracle.setDONPublicKey(
        "0x" + networkConfig[network.name]["functionsPublicKey"]
  ...
  if (process.env.ETHERSCAN_API_KEY || process.env.POLYGONSCAN_API_KEY) {
    ...
    try {
      ...
      console.log(
        "Error verifying contracts.  Delete the ./build folder and try again."
      );
      console.log(error);
    }
  }
  ...
  console.log(
    `FunctionsOracleFactory successfully deployed to ${oracleFactory.address} on ${network.name}\n`
  );
  console.log(
