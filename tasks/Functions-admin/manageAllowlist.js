...
async function addOrRemove(action, taskArgs) {
  ...
  const oracle = oracleFactory.attach(
  ...
  if (action == Action.Add) {
    console.log(
      `Adding addresses ${addresses} to oracle ${
        networkConfig[network.name]["functionsOracle"]
      }`
    );
    tx = overrides
    ...
    console.log(
      `Removing addresses ${addresses} from oracle ${
        networkConfig[network.name]["functionsOracle"]
      }`
    );
    tx = overrides
  ...
  console.log(
    `Waiting ${VERIFICATION_BLOCK_CONFIRMATIONS} blocks for transaction ${tx.hash} to be confirmed...`
  );
  await tx.wait(VERIFICATION_BLOCK_CONFIRMATIONS);

  console.log(
    `Allowlist updated for oracle ${oracle.address} on ${network.name}`
  );
}
...
task(
  ...
  "Remove wallets from allowlist in the Oracle contract"
)
  .addParam("addresses", "Comma-separated list of addresses")
  .setAction(async (taskArgs) => {
